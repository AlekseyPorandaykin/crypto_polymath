// spot_calculator_test.go — unit-тесты спотового калькулятора PnL.
//
// Зачем: спотовый PnL — простейший расчёт (без плеча), но используется
// в UI и отчётах. Тесты проверяют: прибыль, убыток, нулевой PnL,
// edge-case с нулевой ценой входа (деление на 0).
package trading
