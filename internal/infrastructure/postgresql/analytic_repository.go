...
func (repo *AnalyticRepository) Save(ctx context.Context, data ...analysis.Analytic) error {
	...
	dtos := make([]AnalyticDTO, 0, len(data))
	for _, item := range data {
		if math.IsNaN(item.Value) {
			continue
		}
		dtos = append(dtos, AnalyticDTO{
			ID:             item.ID,
			Name:           item.Name,
...
INSERT INTO analytics(
                      ...
                      value,
                      created_at)
VALUES (:id,:name,:exchange,:symbol,:unit,:interval,:datetime,:depth,:by_indicator,:indicator_depth,:value,:created_at)`
	// BindNamed вызывается один раз на весь срез: sqlx сам размножит VALUES(...) на len(dtos) групп
	...
	preparedQuery, args, err := repo.db.BindNamed(query, dtos)
	if err != nil {
		return err
	}
...
WHERE exchange = ?
  ...
  AND datetime IN (?)`, exchangeName, symbol, string(unit), interval, name, indicatorDepth, depth, datetimes)
	...
	if err := repo.db.SelectContext(ctx, &data, repo.db.Rebind(query), args...); err != nil {
		return nil, err
	}
...
`
	...
	err := repo.db.SelectContext(ctx, &data, repo.db.Rebind(query), exchangeName, symbol, unit, interval, name, indicatorDepth, depth)
	if err != nil {
		return nil, err
	}
...
FROM analytics`
	rows, err := repo.db.QueryContext(ctx, repo.db.Rebind(query))
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
